...
<table class="table1" cellspacing="0" cellpadding="5px">
    ...
    @foreach ($groupedItems as $productId => $items)
        ...
        <tr class="height" style="background-color: {{ $item['winning_amount'] > 0 ? '#ffcccc' : 'transparent' }};">
            <td>
            ...
            <td align="right">
                @if($item['is_free'] == 0)
                    @if($item['lot_frac'] != 0)
                        {{ $item['lot_frac'] }} <span>{{ $item['winning_amount'] != 0 ? $item['winning_amount'] : '' }}</span>
                    @else
...
<style>
  ...
  @media print {
    .company-header {
      font-size: 30px !important; /* Enlarge the font size during print to see the effect */
    }
